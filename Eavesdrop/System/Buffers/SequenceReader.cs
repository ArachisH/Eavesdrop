#if NETSTANDARD2_0
// Source: https://raw.githubusercontent.com/dotnet/runtime/ecc8ba280fa3d238b6fa0b7847f693163c6c02c1/src/libraries/System.Memory/src/System/Buffers/SequenceReader.cs
...
public ref partial struct SequenceReader<T> where T : unmanaged, IEquatable<T>
{
    ...
    public SequenceReader(ReadOnlySequence<T> sequence, ReadOnlySequenceSegment<T> startSegment, int startIndex, ReadOnlySequenceSegment<T> endSegment, int endIndex)
    {
        CurrentSpanIndex = 0;
        Consumed = 0;
        Sequence = sequence;
        ...
        CurrentSpan = startSegment.Memory.Span;
        if (startSegment != endSegment)
        {
            CurrentSpan = CurrentSpan.Slice(startIndex);
            _nextPosition = new SequencePosition(startSegment.Next, 0);
        }
        else CurrentSpan = CurrentSpan.Slice(startIndex, endIndex - startIndex);
...
//internal static class CompatibleReadOnlySequenceExtensions
//{
//    [MethodImpl(MethodImplOptions.AggressiveInlining)]
//    internal static void GetFirstSpan<T>(this ReadOnlySequence<T> sequence, ReadOnlySequenceSegment<T> startSegment, ReadOnlySequenceSegment<T> endSegment, out ReadOnlySpan<T> first, out SequencePosition next)
//    {
//        first = default;
//        next = default;
//        object? startObject = _startObject;
//        int startIndex = startSegment.;

//        if (startObject != null)
//        {
//            bool hasMultipleSegments = startSegment != endSegment;
