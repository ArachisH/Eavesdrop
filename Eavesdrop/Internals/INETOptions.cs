...
public static class INETOptions
{
    ...
    public static void Save()
    {
        lock (_stateLock)
        {
            ...
            {
                options[optionsCount++] = new INETOption(OptionKind.INTERNET_PER_CONN_PROXY_SERVER, joinedAddresses);
                if (!string.IsNullOrWhiteSpace(joinedOverrides))
                {
                    options[optionsCount++] = new INETOption(OptionKind.INTERNET_PER_CONN_PROXY_BYPASS, joinedOverrides);
                }
            ...
            var inetPackage = new INETPackage
            {
                ...
                _optionCount = optionsCount
            };

            IntPtr optionsPtr = Marshal.AllocCoTaskMem(_iNetOptionSize * optionsCount);
