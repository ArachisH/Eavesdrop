    ...
    public unsafe static void Save(string? proxyAddress, string? autoConfigPath, bool isApplyingProxyExplicitly = false)
    {
        ...
        lock (_stateLock)
        {
            ...
            fixed (char* proxyAddressPtr = $"http={proxyAddress};https={proxyAddress}")
            {
                ...
                Span<INETOption> options =
                [
                    new INETOption(OptionKind.INTERNET_PER_CONN_FLAGS, (int)kind),
                    new INETOption(OptionKind.INTERNET_PER_CONN_PROXY_SERVER, proxyAddressPtr),
                    new INETOption(OptionKind.INTERNET_PER_CONN_AUTOCONFIG_URL, autoConfigUrlPtr),
                ];
